...
{
  private $current_page   = 'dashboard/evaluasi';
  ...
  function index()
  {
    ...
    $data['evaluasi'] = $this->m_evaluasi->getData();
  ...
  function selectData()
  {
    ...
    $select['id']   = $id;
    $data['select'] = $this->m_evaluasi->selectOne($select);
    $this->output
  ...
  function editData()
  {
    if ($_POST) {
      ...
      $query = $this->m_evaluasi->updateData($data['id'], $data);
      if ($query) {
  ...
  function deleteData()
  {
    $id = $this->uri->segment(4);
    $data['id'] = $id;
    $delete = $this->m_evaluasi->removeData($data);
    if ($delete != false) {
